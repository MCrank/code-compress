...
namespace CodeCompress.Server.Prompts;

[McpServerPromptType]
internal sealed class PromptsProvider
{
    private PromptsProvider() { }

    [McpServerPrompt]
    [Description("Codebase exploration workflow: index_project → project_outline → search_symbols → get_symbol")]
    public static ChatMessage ExploreCodebase() =>
        new(ChatRole.User, """
    ...
    [McpServerPrompt]
    [Description("Impact analysis workflow: index_project → blast_radius → find_references → dependency_graph")]
    public static ChatMessage FindImpact() =>
        new(ChatRole.User, """
    ...
    [McpServerPrompt]
    [Description("Change review workflow: snapshot_create → [make changes] → index_project → changes_since")]
    public static ChatMessage ReviewChanges() =>
        new(ChatRole.User, """
    ...
    [McpServerPrompt]
    [Description("Targeted symbol debugging: search_symbols → get_hot_path → get_symbol → find_references")]
    public static ChatMessage DebugSymbol() =>
        new(ChatRole.User, """
