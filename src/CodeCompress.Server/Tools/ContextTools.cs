...
internal sealed class ContextTools
{
    ...
    [McpServerTool(Name = "assemble_context", Title = "Assemble Context", ReadOnly = true, Destructive = false, Idempotent = true, OpenWorld = false)]
    [Description("One-shot context assembly — searches symbols, retrieves source code, and builds a structured overview within a token budget. Use INSTEAD of manually calling search_symbols + get_symbol + project_outline when starting a task. Reduces 5-10 tool round-trips to 1. ~1,000–40,000 tokens (controlled by budget parameter, default 40,000). Prefer over separate search_symbols + get_symbol calls when assembling context for a new task. Returns Markdown: file tree overview, source code sections grouped by file (with syntax-highlighted fenced blocks), and a metadata footer showing token usage. Large symbols (>16KB) are automatically summarized with signature + child list — use expand_symbol for individual methods. Set activeFile to prioritize the file you're editing. Requires index_project first. Zero-result response is JSON: {query, total_matches: 0, hint}. Errors return JSON {error, code, retryable}. Codes: INVALID_PATH, EMPTY_QUERY, FTS5_QUERY_ERROR, INVALID_PATH_FILTER. Next: expand_symbol or get_hot_path for targeted follow-up on specific methods.")]
