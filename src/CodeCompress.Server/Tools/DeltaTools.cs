...
internal sealed partial class DeltaTools
{
    ...
    [Description("Get an annotated directory tree with file counts and line counts per directory. Does NOT require index_project — reads the filesystem directly. Use to quickly understand project structure before indexing. Returns plain text: indented tree with 'dirname/ (N files, N lines)' per directory and 'filename (N lines)' per file. Errors return JSON {error, code}. Codes: INVALID_PATH, DIRECTORY_NOT_FOUND.")]
