...
internal sealed class QueryTools
{
    ...
    public async Task<string> SearchSymbols(
        ...
        [Description("Enable fuzzy (typo-tolerant) matching using Levenshtein distance ≤ 2. Merges fuzzy candidates with FTS5 results. Default false. Use when exact or token-split search returns no results for a known symbol name with a possible typo.")] bool fuzzy = false,
        CancellationToken cancellationToken = default)
    {
        string validatedPath;
        try
